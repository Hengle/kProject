    ...
    public class QHierarchy
    {
        ...
        private static float drawTagLayerText(GameObject gameObject, Rect rect)
        {
            ...
            if (!showAlways && tag == "Untagged" && layer == 0)
            {
                ...
                {
                    rect.width = 4 + QHierarchySettings.getSetting<int>(QHierarchySetting.TagAndLayerSizeValue);
                    rect.x -= rect.width;
                    EditorGUI.DrawRect(rect, backgroundColor);
                    rect.x += rect.width;
                }
            ...
            else
            {
                ...
                {
                    rect.width = QHierarchySettings.getSetting<int>(QHierarchySetting.TagAndLayerSizeValue);
                }
